...
public class BadgeDrawable extends Drawable {
    ...
    public BadgeDrawable(Context context) {
        float mTextSize = 110;//context.getResources().getDimension(R.dimen.badge_text_size);

        mBadgePaint = new Paint();
        mBadgePaint.setColor(ContextCompat.getColor(context.getApplicationContext(), R.color.greenYLP));
    ...
    public void draw(Canvas canvas) {
        ...
        Log.d(TAG, "draw: "+bounds.toString());
        /*Using Math.max rather than Math.min */

        float radius = ((Math.max(width, height) / 2)) / 2;
        float centerX = (width - radius - 1) +80;
        float centerY = radius -30;
        if(mCount.length() <= 2){
            // Draw badge circle.
...
//	        	canvas.drawRoundRect(radius, radius, radius, radius, 10, 10, mBadgePaint);
        ...
        Log.d(TAG, "draw: "+mTxtRect.bottom+" "+mTxtRect.top );
