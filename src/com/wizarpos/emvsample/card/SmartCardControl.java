	...
	public boolean open()
	{
		...
		if(cardType == CARD_CONTACT)
		{
			if(readerSlot >= 0)
			{
				...
				{
					slotState = SLOT_OPEN;
					if(debug)Log.d(APP_TAG, "ContactICCardReaderInterfaceYLP.open(" + readerSlot + ") OK");
					return true;
				}
			}
		}
		...
		{ 
			...
			if(debug)Log.d(APP_TAG, "ContactlessICCardReaderInterfaceYLP.open() result = " + nResult);
			if(nResult >= 0)
			{
				slotState = SLOT_OPEN;
				return true;
		...
		if(cardType == CARD_CONTACT)
		{
			if(slotState == SLOT_OPEN)
			{
				...
				if(nResult >= 0)
				{
					...
					if(debug)
					{
						//Log.d("(APP_TAG, "ContactICCardReaderInterfaceYLP.powerOn(" + readerSlot + ") OK");
	...
	public void close()
	{
		...
		if(slotState != SLOT_CLOSE)
		{
			...
			if(cardType == CARD_CONTACT)
			{
				...
				if(debug)Log.d(APP_TAG, "ContactICCardReaderInterfaceYLP.close(" + readerSlot + ")");
				readerHandle = -1;
			}
			else if(cardType == CARD_CONTACTLESS)
			{ 
				ContactlessICCardReaderInterface.close();
				if(debug)Log.d(APP_TAG, "ContactlessICCardReaderInterfaceYLP.close(" + readerSlot + ")");
			}
		}
	}
